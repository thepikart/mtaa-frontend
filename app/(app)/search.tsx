...
export default function SearchScreen() {
  ...
  const loadData = useCallback(async (q: string, cat: string | null) => {
    ...
    try {
      let data;
      if (cat)           data = await EventService.getEventsByCategory(cat, 20);
      else if (q.trim()) data = await EventService.searchEvents(q);
      else               data = await EventService.getUpcomingEvents();
      setResults(data);
    } catch (err) {
